...
func LexKeyWhiteSpace(l *Lexer) StateFunc {
	...
	default:
		return l.Errorf("missing equal sign")
	}
}

func LexValueWhiteSpace(l *Lexer) StateFunc {
	l.Next()
	l.Start = l.Pos

	switch next := l.PeekNext(); next {
	case RuneTab, RuneSpace:
		return LexValueWhiteSpace
	default:
		return LexValue
	}
}
...
func LexEqualSign(l *Lexer) StateFunc {
	l.Pos += len(string(RuneEqualSign))
	l.Emit(TokenEqualSign)
